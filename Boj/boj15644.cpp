...
char a[N][N]; int n, m;  vector<DATA> v;
DATA color[2];   bool Red, Blue;  char Color[] = { 'R','B' };
int ans = INT; vector<char> dir, route;
...
void dfs(int cnt, int Before) {
	...
	for (int i = 0; i < 4; i++) {
		if (Before != i) {
			if (!i) dir.push_back('U');
			if (i == 1) dir.push_back('D');
			if (i == 2) dir.push_back('R');
			if (i == 3) dir.push_back('L');
